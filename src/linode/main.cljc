(ns linode.main
  "Kotodama WASM entrypoint for the Linode clean-room actor (L4) — Clojure port.
  ...
  facts (`linode.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "linode")
(def tier "L4")
...
(def entity-specs
  [{:entity "Instance"    :plural "instances"     :id-prefix "linode_ins"
    :fields [:label :status :region :type :image :created :updated]     :required [:label]
    :coerce {}                       :refs {}}
   {:entity "Volume"      :plural "volumes"       :id-prefix "linode_vol"
    :fields [:label :status :size :region :encryption :created :updated] :required [:label]
    :coerce {:size :int}             :refs {}}
   {:entity "Domain"      :plural "domains"       :id-prefix "linode_dom"
    :fields [:domain :type :status :soaEmail :created]                   :required [:domain]
    :coerce {}                       :refs {}}
   {:entity "NodeBalancer" :plural "nodebalancers" :id-prefix "linode_nod"
    :fields [:label :type :region :ipv4 :ipv6 :created :updated]         :required [:label]
    :coerce {}                       :refs {}}
   {:entity "LKECluster"  :plural "lkeclusters"   :id-prefix "linode_lke"
    :fields [:label :region :k8sVersion :tier :created :updated]         :required [:label]
    :coerce {}                       :refs {}}
   {:entity "Image"       :plural "images"        :id-prefix "linode_ima"
    :fields [:label :status :type :size :isPublic :created :updated]     :required [:label]
    :coerce {:size :int :isPublic :bool} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"linode.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "linode-compat" :tier tier :entities entities} 200])
...
(defn start! [] :linode-compat/ready)
